...
public class SequenceFileValueInputStream implements RecordInputStream {

    private SequenceFile.Reader _reader;
    private BytesWritable writable = new BytesWritable();

    public SequenceFileValueInputStream(FileSystem fs, Path path) throws IOException {
        _reader = new SequenceFile.Reader(fs, path, fs.getConf());
    }

    public byte[] readRawRecord() throws IOException {
        boolean gotnew = _reader.next(NullWritable.get(), writable);
        if (!gotnew) {
            return null;
        }
